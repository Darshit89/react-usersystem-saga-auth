...
console.log("activityDetail",activityDetail)
  const generalRowSelection = {
    selectedRowKeys:activityDetail?.activity?.generalRow.map((item:any)=>item.key),
    getCheckboxProps: (record: DataType) => ({
      // disabled: activityDetail?.activity?.generalRow.find((item:any)=>item.key != record.key), // Column configuration not to be checked
      name: record.general,
    }),
  };

  const trainingRowSelection = {
    selectedRowKeys: activityDetail?.activity?.trainingRow.map((item:any)=>item.key),
    getCheckboxProps: (record: DataType) => ({
    // disabled: activityDetail?.activity?.trainingRow.find((item:any)=>item.key != record.key), // Column configuration not to be checked          
      name: record.training,
    }),
  };

  const consultationRowSelection = {
    selectedRowKeys: activityDetail?.activity?.consultationRow.map((item:any)=>item.key),
    getCheckboxProps: (record: DataType) => ({
      // disabled: activityDetail?.activity?.consultationRow.find((item:any)=>item.key != record.key), // Column configuration not to be checked
